...
export default async function handler(req, res) {
  ...
  try {
    // Verifica em transação se os números ainda estão disponíveis
    ...
    await db.runTransaction(async (t) => {
      ...
      for (const n of numbers) {
        if (current[n] && current[n] !== 'available') {
    ...
    const mpResponse = await payment.create({
      body: {
        ...
        notification_url: `${process.env.VERCEL_URL || process.env.BASE_URL}/api/webhook`,
        external_reference: reservationRef.id,
      },
    });

    // Salva ID do pagamento na reserva
